...
/*   Updated: 2025/03/17 15:28:30 by qliso            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    draw_background(t_scene *scene)
{
    draw_rect(scene->img, (t_rect){0, 0, WINDOW_WIDTH, WINDOW_HEIGHT}, GRAY);
    draw_rect(scene->img, (t_rect){scene->gameOx, scene->gameOy, GAME_WIDTH, GAME_HEIGHT}, DARK);
}
...
void    draw_walls(t_scene *scene, t_raycast *ray, int color)
{
    ...
    ray->ca = scene->p_a - ray->ra;
    if (ray->ca < 0)
        ray->ca += 2 * PI;
    if (ray->ca > 2 * PI)
        ray->ca -= 2 * PI;
    lineH = scene->map_s / (ray->ray_dist * cos(ray->ca)) * 160;
    lineO = - lineH / 2 + scene->gameOy / 2 + 100;
    startY = clamp(lineO, scene->gameOy, 1000);
    endY = lineH + scene->gameOy;
    draw_rect(scene->img, (t_rect){ray->r * 8 + scene->gameOx - scene->map_x, startY, 8, endY}, color);


    // lineO = lineH / 4;

    // lineH = clamp((scene->map_s * 320) / ray->ray_dist, -10000, 320);
    // lineO = 100 - lineH / 4;
    // draw_rect(scene->img, (t_rect){ray->r * 8 + 530, 0, 8, lineO - 1}, BLUE);
    // draw_rect(scene->img, (t_rect){ray->r * 8 + 530, lineH + lineO + 1, 8, 320 - lineH - lineO - 1}, GRASS);
}
